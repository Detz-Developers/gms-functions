...
describe("Firebase Users Rules", () => {
  const usersRef = (uid: string) => ref(db, `users/${uid}`);

  it("blocks non-admin from creating user profile", () => {
    login("operator@gmail.com", "operator123")
      .then(() =>
        set(usersRef("U999"), {
      ...
      .then(() => {
        throw new Error("Non-admin should not create user profile!");
      })
      .catch((err: any) => {
  ...
  it("allows admin to create user profile", () => {
    login("admin@gmail.com", "Temp@123")
      ...
      .then(() => {
        cy.log("Admin write success!");
      })
      .catch((err: any) => {
        if (err.code === "auth/invalid-credential") {
          throw new Error(
            "Login failed: Check admin email/password in Firebase Auth"
          );
        } else {
          throw new Error("Admin write failed: " + err.message);
        }
      });
  });
});
